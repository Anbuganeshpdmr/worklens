...
const UserManagement = () => {
  ...
  const loadRoles = async () => {
    try {
      ...
      let list = Array.isArray(res) ? res : Array.isArray(res?.roles) ? res.roles : Array.isArray(res?.data) ? res.data : [];
      setRolesList(list);
    } catch (err) {
  ...
  const filteredUsers = users.filter((u) => {
    ...
    const isActive = u?.active === true || u?.active === 1 || u?.isActive === true;
    const matchStatus =
      statusFilter === "All" ||
  ...
  const handleSave = async (userData) => {
    try {
      ...
      if (modal.user) {
        const id = modal.user.id ?? modal.user.userId ?? modal.user._id ?? modal.user.empId;
  ...
  const handleAddRole = async (e) => {
    ...
    try {
      ...
      setRoleModal(false); setNewRoleName("");
  ...
  const handleUpdateRole = async (e) => {
    ...
    try {
      ...
      setRolesList((p) => p.map((r) => (r.id === editingRole.id ? { ...r, name } : r)));
      setEditingRole(null);
    } catch (err) { console.error(err); }
    finally { setRoleSaving(false); }
  };
                ...
                <div className="um-panel-right">
                  <div className="um-search">
                    <svg className="um-search-icon" viewBox="0 0 20 20" fill="none">
                      <circle cx="9" cy="9" r="6" stroke="#98a2b3" strokeWidth="1.5" />
                      <path d="M14 14l3 3" stroke="#98a2b3" strokeWidth="1.5" strokeLinecap="round" />
                    </svg>
                    <input
                      type="text"
                      value={search}
              ...
              {!loading && (
                <div className="um-grid">
                  {filteredUsers.length > 0 ? (
                    filteredUsers.map((u) => (
                      <MemberCard key={u.id || u.empId} user={u} onEdit={handleEdit} />
                    ))
                  ) : (
          ...
          {activeTab === "roles" && (
            <div className="um-panel">
              <div className="um-panel-header">
                ...
                <div className="um-panel-right">
                  <button className="um-add-btn" onClick={() => setRoleModal(true)}>
                    + Add Role
                  </button>
                </div>
              </div>

              {rolesLoading && <div className="um-loading">Loading roles...</div>}
                  ...
                  {rolesList.map((r, i) => {
                    ...
                    return (
                      <div key={r.id ?? i} className="um-role-item">
                        <span className="um-role-badge">{name.charAt(0).toUpperCase()}</span>
                        ...
                        <button
                          ...
                          onClick={() => setEditingRole({ id: r.id ?? i, name })}
                        >
          ...
          <div className="user-modal" style={{ maxWidth: "420px" }}>
            <div className="modal-header">
              ...
              <button type="button" className="close-btn"
                onClick={() => { setRoleModal(false); setNewRoleName(""); }}>×</button>
            </div>
            <form className="user-form" onSubmit={handleAddRole}>
              <div className="form-group">
                <label>Role Name <span style={{ color: "#f04438" }}>*</span></label>
                <input
              ...
              <div className="form-actions">
                <button type="button" disabled={roleSaving}
                  onClick={() => { setRoleModal(false); setNewRoleName(""); }}>Cancel</button>
        ...
        <div className="modal-overlay">
          <div className="user-modal" style={{ maxWidth: "420px" }}>
            ...
            <form className="user-form" onSubmit={handleUpdateRole}>
              ...
              <div className="form-actions">
                <button type="button" disabled={roleSaving}
                  onClick={() => setEditingRole(null)}>Cancel</button>
                <button type="submit" disabled={roleSaving}>
